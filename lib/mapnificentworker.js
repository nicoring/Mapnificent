...
process.on('message', mapnificentWorker);
var stationMap, stations, lines, reportInterval; // "global" variables

console.log("child process runnning");

function calculateTimes(nextStations, lines, secondsPerKm, maxWalkTime) {
  'use strict';
  ...
  while (nsl > 0){ // as long as we have next stations to go
    for (i = 0; i < nsl; i += 1){
      ...
      if (reportInterval !== 0 && count % reportInterval === 0){
        process.send({status: 'working', at: count});
      }
...
function mapnificentWorker(data) {
  'use strict';

  console.log("mapnificent worker doing his work...");

  stationMap = {};
  stations = data.stations;
  lines = data.lines;
  reportInterval = data.reportInterval;
  var fromStations = data.fromStations,
    distances = data.distances,
    maxWalkTime = data.maxWalkTime,
    secondsPerKm = data.secondsPerKm,
  ...
  */
  ...
  process.send({status: 'done', stationMap: stationMap});
  // process.disconnect();
}
// }
