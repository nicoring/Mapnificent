...
function MapnificentPosition(mapnificent, latlng, time) {
  ...
  this.stationsByDistanceAndTime = null;
  this.stationsAABB = null;
...
MapnificentPosition.prototype.createWorker = function(){
  ...
  this.worker = child_process.fork(__dirname + '/mapnificentworker');
...
MapnificentPosition.prototype.getReachableStations = function(stationsAround) {
  ...
  var updateAabb = function(station) {
    if (station.lat - station.radius < aabb.south) {
      aabb.south = station.lat - station.radius;
    }

    if (station.lat + station.radius > aabb.north) {
      aabb.north = station.lat + station.radius;
    }

    if (station.lng - station.radius < aabb.west) {
      aabb.west = station.lng - station.radius;
    }

    if (station.lng + station.radius > aabb.east) {
      aabb.east = station.lng + station.radius;
    }
  };
  ...
  aabb.north = station.lat;
  aabb.east = station.lng;
  aabb.south = station.lat;
  aabb.west = station.lng;
  updateAabb(station);

  for (var i = 0; i < stationsAround.length && this.worker !== null; i++) {
  ...
  return _.filter(points, function(point) {
    ...
    for (var circle in self.stationsByDistanceAndTime) {
      inside = Geo.isPointInCircle(
        lng, // point
        lat, 
        self.stationsByDistanceAndTime[circle].lng, // circle
        self.stationsByDistanceAndTime[circle].lat, 
